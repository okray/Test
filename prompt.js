...
(function( $ ) {
  ...
  $.fn.promptumenu = function(options) {
	...
	return this.each(function () {
	    (new $.promptumenu($(this), settings));
	});

  };

  $.promptumenu = function (element, settings) {
      var $this = $(element);
      var properties;
      var cursor = {
          x: 0,
      ...
      var methods = {
          ...
          go_to: function (index, easing, webkit) {
              ...
              checkStatus($this.parent('div.promptumenu_window').find('.prePage a'),
                          $this.parent('div.promptumenu_window').find('.nextPage a'));
          },
        
      };

      var checkStatus = function (pre, next) {
          if (cells.current_page == 1 || cells.pages == 1) {
              $(pre).hide();              
          }
          else {
              $(pre).show();
          ...
          if (cells.pages == 1 || cells.current_page == cells.pages) {              
              $(next).hide();
          }
          else {
              $(next).show();
      ...
      var resetList = function () {
          ...
          if (settings.pages == true) {
              if (cells.pages > 1) {
                  ...
                  //if ($this.parent('div.promptumenu_window').find('.promptumenu_nav').length != 0) {

                  //    $this.parent('div.promptumenu_window').find('.promptumenu_nav').remove();
                  //}
                  ...
                  $this.parent('div.promptumenu_window').append('<div class="prePage"><a></a></div>');
                  $this.parent('div.promptumenu_window').find('.prePage a').bind('click', function () {
                      methods.go_to(cells.current_page - 1);
                  });

                  $this.parent('div.promptumenu_window').append('<div class="nextPage"><a></a></div>');
                  $this.parent('div.promptumenu_window').find('.nextPage a').bind('click', function () {
                      methods.go_to(cells.current_page + 1);
                  });
              }
              else {
              ...
              if (cells.current_page > cells.pages) {
                  cells.current_page = cells.pages;
              }
              methods.go_to(cells.current_page);
          }
      ...
      this.isLastPage = function() {
          if (cells.pages == cells.current_page) {
              return true;
          }
          else {
              return false;
